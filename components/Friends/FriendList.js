...
function FriendList({ friendList, setClicked }) {
  const mappedFriends = friendList.map((friend, index) => {
    return (
      <Group  className="border-b-2 pb-1 pt-50 mb-5">
  ...
  return (
    <ScrollArea offsetScrollbars scrollbarSize={8} className='h-[85%] sm:h-[72%] mt-2'>
      {mappedFriends}
      <div>asd </div>
      <div>asd </div>
      <div>asd </div>
